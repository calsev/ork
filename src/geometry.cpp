...
const glm::dmat3x3
    zero_mat(glm::dvec3(0.0, 0.0, 0.0), glm::dvec3(0.0, 0.0, 0.0), glm::dvec3(0.0, 0.0, 0.0));
/*
These are transform matrices into MC view LCS.
As always, these are column major.
*/
const glm::dmat3x3
    to_top(glm::dvec3(1.0, 0.0, 0.0), glm::dvec3(0.0, 1.0, 0.0), glm::dvec3(0.0, 0.0, 1.0));
const glm::dmat3x3
    to_front(glm::dvec3(1.0, 0.0, 0.0), glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(0.0, -1., 0.0));
const glm::dmat3x3
    to_back(glm::dvec3(-1., 0.0, 0.0), glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(0.0, 1.0, 0.0));
const glm::dmat3x3 to_bottom(
    glm::dvec3(-1., 0.0, 0.0),
    glm::dvec3(0.0, 1.0, 0.0),
    glm::dvec3(0.0, 0.0, -1.));
const glm::dmat3x3
    to_right(glm::dvec3(0.0, 1.0, 0.0), glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(1.0, 0.0, 0.0));
const glm::dmat3x3
    to_left(glm::dvec3(0.0, -1., 0.0), glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(-1., 0.0, 0.0));
...
const glm::dmat3x3
    from_top(glm::dvec3(1.0, 0.0, 0.0), glm::dvec3(0.0, 1.0, 0.0), glm::dvec3(0.0, 0.0, 1.0));
const glm::dmat3x3
    from_front(glm::dvec3(1.0, 0.0, 0.0), glm::dvec3(0.0, 0.0, -1.), glm::dvec3(0.0, 1.0, 0.0));
const glm::dmat3x3
    from_back(glm::dvec3(-1., 0.0, 0.0), glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(0.0, 1.0, 0.0));
const glm::dmat3x3 from_bottom(
    glm::dvec3(-1., 0.0, 0.0),
    glm::dvec3(0.0, 1.0, 0.0),
    glm::dvec3(0.0, 0.0, -1.));
const glm::dmat3x3
    from_right(glm::dvec3(0.0, 0.0, 1.0), glm::dvec3(1.0, 0.0, 0.0), glm::dvec3(0.0, 1.0, 0.0));
const glm::dmat3x3
    from_left(glm::dvec3(0.0, 0.0, -1.), glm::dvec3(-1., 0.0, 0.0), glm::dvec3(0.0, 1.0, 0.0));
...
bool points_advance_clockwise(const glm::dvec2& begin, const glm::dvec2& end, const glm::dvec2& center)
{
    ...
    if(GLM::equal(glm::length2(dir_begin), 0.) ||
       GLM::equal(glm::length2(dir_end), 0.)) {
        ORK_THROW(ORK("Cannot compute the angle between a center and itself"));
    }

    const double rad = glm::orientedAngle(
        glm::normalize(dir_begin), glm::normalize(dir_end));
    return rad < 0.;
}
bool points_advance_clockwise(
    const glm::dvec3& begin,
    const glm::dvec3& end,
    const glm::dvec3& center,
    const GLM::dunit3& normal)
{
    const glm::dvec3 start_dir(glm::normalize(begin - center));
    const glm::dvec3 end_dir(glm::normalize(end - center));
    if(GLM::equal(glm::length2(start_dir), 0.) ||
       GLM::equal(glm::length2(end_dir), 0.)) {
        ORK_THROW(ORK("Cannot compute the angle between a center and itself"));
    }
...
bounding_box::bounding_box(const glm::dvec3& v1, const glm::dvec3& v2)
    : _min_pt(glm::min(v1, v2))
    , _max_pt(glm::max(v1, v2))
{}
...
{
    const interval limits = project_box_on_axis(
        _min_pt, _max_pt, GLM::dunit3(direction));
    return limits.max();
}
...
{
    return std::min(
        std::min(_max_pt.x - _min_pt.x, _max_pt.y - _min_pt.y),
        _max_pt.z - _min_pt.z);
}
double bounding_box::volume() const
{
    return (_max_pt.x - _min_pt.x) * (_max_pt.y - _min_pt.y) *
           (_max_pt.z - _min_pt.z);
}
...
{
    return bounding_box(
        glm::min(lhs.min_pt(), rhs.min_pt()), glm::max(lhs.max_pt(), rhs.max_pt()));
}
...
{
    string_stream strm;
    strm << ORK("[(") << box.min_pt().x << ORK(", ") << box.min_pt().y << ORK(", ")
         << box.min_pt().z << ORK("), (") << box.max_pt().x << ORK(", ")
         << box.max_pt().y << ORK(", ") << box.max_pt().z << ORK(")]");
    return strm.str();
}
