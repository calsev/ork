...
public:
    impl()
        : data{}
    {}
    explicit impl(const glm::dvec3& vec)
        : data{vec}
    {}
    impl(const double x, const double y, const double z)
        : data{x, y, z}
    {}
};
...
vector::vector()
    : _pimpl{new impl()}
{}
vector::vector(const glm::dvec3& vec)
    : _pimpl{new impl(vec)}
{}
vector::vector(const GLM::dunit3& vec)
    : _pimpl{new impl(vec.get())}
{}
vector::vector(const double x, const double y, const double z)
    : _pimpl{new impl(x, y, z)}
{}
...
string vector::as_string() const
{
    ...
    stream << ORK("(") << to_dimension(_pimpl->data.x) << ORK(", ")
           << to_dimension(_pimpl->data.y) << ORK(", ")
           << to_dimension(_pimpl->data.z) << ORK(")");
    return stream.str();
}
...
void vector::export_xml(xml::node& node) const
{
    ...
    node.append_attribute(BORK("y")).set_value(
        ORK_STR_2_BYTE(to_dimension(_pimpl->data.y)).c_str());
    node.append_attribute(BORK("z")).set_value(
        ORK_STR_2_BYTE(to_dimension(_pimpl->data.z)).c_str());
}
#    endif
#    if ORK_USE_YAML
