...
static const char_t* const oriention_strings[] =
    {ORK("+X"), ORK("-X"), ORK("+Y"), ORK("-Y"), ORK("+Z"), ORK("-Z")};
...
const char_t* orientation2string(const orientation o)
{
    ...
    if(index < sizeof(oriention_strings) / sizeof(*oriention_strings))
        return oriention_strings[index];
    else
        ORK_THROW(ORK("Attempt to convert invalid orientation"));
}
