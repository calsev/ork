...
tagger::tagger(const string& tag)
    : _pimpl{new impl(tag)}
{}
tagger::tagger(const string& tag, bool numbered_folders)
    : _pimpl{new impl(tag, numbered_folders)}
{}
tagger::~tagger() {}
...
setup_hierarchy::setup_hierarchy(const string& setup_root, const string& as_is_path, const string& to_be_path)
    : _pimpl{new impl{}}
{
    if(!file::ensure_directory(setup_root))
        ORK_THROW(ORK("Setup root could not be created: ") << setup_root);
...
{
    file::path setup_dir;
    if(!top_subdirectory(get_path(), setup_dir))
        ORK_THROW(ORK("Setup directory not found for root: ") << get_path());
...
struct setup_match {
    string setups;
    file::path setup_dir;
    setup_match(const string& exact_setups)
        : setups(exact_setups)
        , setup_dir()
    {}
    ...
    {
        if(!setup_dir.empty())
            return; // Already found (this could be a superstring, so find might work)
        if(!ext_file::is_directory(p))
            return; // Just in case
        const string dir_name(p.stem().ORK_GEN_STR());
        if(dir_name.find(setups) != string::npos)
            setup_dir = p;
    }
};
...
file::path setup_hierarchy::get_subdirectory(const string& exact_setups)
{
    ...
    if(match.setup_dir.empty())
        ORK_THROW(ORK("Setup permutation not found: ") << exact_setups);
    _pimpl->cached = false; // Dirty, in case the client fetches more than one setup
...
{
    return do_get_setups(
        file::canonical_normalized_trimmed(get_subdirectory()).filename().ORK_GEN_STR());
}

const std::vector<orientation>& setup_hierarchy::get_setups(const string& exact_setups)
{
    return do_get_setups(file::canonical_normalized_trimmed(get_subdirectory(exact_setups))
                             .filename()
                             .ORK_GEN_STR());
}
...
const std::vector<orientation>& setup_hierarchy::do_get_setups(const string& setup_dir)
{
    if(!_pimpl->cached) {
        ...
        if(_pimpl->setups.empty())
            ORK_THROW(
                ORK("Setup directory not formatted correctly: ")
                << get_path() << ORK("/") << setup_dir);

        _pimpl->cached = true;
    }
    if(_pimpl->setups.empty())
        ORK_THROW(ORK("Setups not found for directory: ") << setup_dir);
