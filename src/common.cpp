...
backtrace generate_backtrace(const unsigned skip_frames)
{
    ...
    const unsigned short num_addr = CaptureStackBackTrace(
        skip_frames + 1, capture_frames, addrs, nullptr);
    ...
    SYMBOL_INFO* symbol = static_cast<SYMBOL_INFO*>(
        malloc(sizeof(SYMBOL_INFO) + sizeof(char) * max_name_len));
...
bstring backtrace::str() const
{
    ...
    for(const stack_frame& sf : frames) {
        stream << index++ << BORK(": ") << sf.function << BORK(" (")
               << sf.filename << BORK(":") << sf.line << BORK(")\n");
    }
...
const char* const utf8_bom_str = reinterpret_cast<char*>(
    utf8_bom_.data()); // Reinterpret cast used to handle pos/neg chars
...
{
    return ext_file::exists(file_or_directory) &&
           ext_file::is_directory(file_or_directory);
}
...
{
    if(!ensure_directory(file))
        return false;
    if(file.empty() || ext_file::exists(file))
        return true;
    if(!ext_file::is_regular_file(file))
        return false;
...
bool top_subdirectory(const file::path& dir, file::path& p)
{
    ...
    std::copy(
        ext_file::directory_iterator(dir),
        ext_file::directory_iterator(),
...
{
    impl::lock_type lock(_pimpl->mutex);
    return _pimpl->converter.to_bytes(s);
}
bstring string_converter_type::wide2byte(const wchar_t* s)
{
    impl::lock_type lock(_pimpl->mutex);
...
{
    impl::lock_type lock(_pimpl->mutex);
    return _pimpl->converter.to_bytes(first, last);
}

wstring string_converter_type::byte2wide(const char s)
{
    impl::lock_type lock(_pimpl->mutex);
    return _pimpl->converter.from_bytes(s);
}
wstring string_converter_type::byte2wide(const char* s)
{
    impl::lock_type lock(_pimpl->mutex);
...
{
    impl::lock_type lock(_pimpl->mutex);
    return _pimpl->converter.from_bytes(first, last);
}
