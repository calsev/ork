...
public:
    ...
    {
        return uniform_prob() < prob ||
               prob == 1.; // 0 and 1 are tricky, we need to handle true 0% and 100& prob
    }
    ...
    template<typename T>
    T uniform_index(const T min, const T max)
    {
    ...
    template<typename T>
    size_t discrete_index(const std::vector<T>& likelihoods)
    {
