...
void command_handler::call_add_options()
{
    _desc.add_options()(
        BORK("help,h"),
        BORK("Produce help message")); // Short option aliases do not work until notify, so we can't use them for help
    add_options(_desc);
...
bool command_handler::process_commands(const options::basic_parsed_options<char_t>& ops)
{
    try {
        ...
        for(auto& option : _desc.options()) {
            ...
            options << ORK("\n  - ") << ORK_BYTE_2_STR(option_name) << ORK(": ")
                    << option_value;
        }
...
bool command_handler::operator()(const int argc, const char_t* const argv[])
{
    try {
        call_add_options();
        options::basic_parsed_options<char_t> ops(
            options::parse_command_line<char_t>(argc, argv, _desc));
...
operator()(const int argc, const char_t* const argv[], const bstring& positional_op)
{
    try {
        call_add_options();
        ...
        options::basic_parsed_options<char_t> ops(
            command_line_parser(argc, argv).options(_desc).positional(p).run());
        return process_commands(ops);
    }
    COMMAND_CATCH
}
...
bool command_handler::operator()(const bstring& env_prefix)
{
    try {
        call_add_options();
        options::basic_parsed_options<char_t> ops(
            options::parse_environment(_desc, env_prefix));
        return process_commands(ops);
    }
    COMMAND_CATCH
}
...
bool command_handler::operator()(std::istream& config_file)
{
    try {
        call_add_options();
        options::basic_parsed_options<char_t> ops(
            options::parse_config_file(config_file, _desc));
        return process_commands(ops);
    }
    COMMAND_CATCH
}
