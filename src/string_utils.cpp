...
#define ORK_STRING_CONVERSION_DEF(TYPE, TO_B, TO_W, SB, SW) \
    ...
    template<> \
    TYPE from_string<TYPE>(const bstring& str) \
    { \
        return ORK_CAT(to_, TYPE)(str); \
    } \
    template<> \
    TYPE from_string<TYPE>(const wstring& str) \
    { \
        return ORK_CAT(to_, TYPE)(str); \
    }
...
template<>
bstring from_string<bstring>(const bstring& str)
{
    return str;
}
template<>
bstring from_string<bstring>(const wstring& str)
{
...
template<>
wstring from_string<wstring>(const bstring& str)
{
    return ork::g_string_converter().byte2wide(str);
}
template<>
wstring from_string<wstring>(const wstring& str)
{
    return str;
}
...
/*
Base 64 codes are an arbitrary list of printable, non-whitespace, ASCII
characters. However, this particular encoding is MIME-compatible.
*/
...
std::array<unsigned char, 4> bytes2chars(const std::array<unsigned char, 3>& buf3)
{
    ...
    std::array<int, 4> vals = {
        ...
        ((buf3[0] & 0x03) << 4) +
            ((buf3[1] & 0xf0) >>
             4) // prev [1...0] shifted to [5...4], and [7...4] shifted to [3...0]
        ,
        ((buf3[1] & 0x0f) << 2) +
            ((buf3[2] & 0xc0) >>
             6) // prev [3...0] shifted to [5...2], and [7...6] shifted to [1...0]
        ,
        buf3[2] & 0x3f // prev [5...0] stay put
    };
    return {static_cast<unsigned char>(vals[0]),
...
std::array<unsigned char, 3> chars2bytes(const std::array<unsigned char, 4>& buf4)
{
    std::array<int, 3> vals = {
        ...
        ((buf4[1] & 0xf) << 4) +
            ((buf4[2] & 0x3c) >>
             2) // prev [3...0] shifted to [7...4], and [5...2] shifted to [3...0]
        ,
...
*/
const bstring codes_hex = BORK("0123456789ABCDEF");
...
void encode_bytes(const size_t byte_index, std::array<unsigned char, 3>& buf3bytes, std::string& retval)
{
    ...
    LOOPI(byte_index + 1)
    { // There is always one more byte than the index
        retval += codes_64[buf4chars[i]]; // Encode the chars
    }
    LOOPI(3 - byte_index)
    { // Add (3 - byte_index) padding bytes
        retval += fill_char;
    }
}
...
bstring encode(const unsigned char* str, const size_t size, const encoding enc)
{
    ...
    std::array<unsigned char, 3> buf3bytes = {
        0, 0, 0}; // Holds the current bytes (3 per 4-character set)
...
void decode_bytes(const size_t byte_index, std::array<unsigned char, 4>& buf4chars, std::string& retval)
{
    ...
    LOOPI(4)
    { // Decode the chars
        buf4chars[i] = static_cast<unsigned char>(
            codes_64.find(static_cast<char>(buf4chars[i])));
    }
