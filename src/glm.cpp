...
{
    if(GLM::parallel(normal, new_normal))
        return vec; // Special case, needed to prevent undefined cross product
    if(GLM::antiparallel(
           normal, new_normal)) { // Special case, needed to prevent undefined cross product
    ...
    const glm::dvec3 right =
        glm::normalize(glm::cross(normal.get(), new_normal.get()));
    const glm::dvec3 left = glm::normalize(glm::cross(new_normal.get(), normal.get()));
    if(!equal(right, -left))
        ORK_THROW(ORK("Orthogonal vector calculation was not stable"));
    ...
    if(!equal(retval, oriented))
        ORK_THROW(ORK("Rotation and orientation were not equivalent"));
...
glm::dvec3 proj_on_plane(const glm::dvec3& vec, const GLM::dunit3& normal)
{
    ...
    if(!equal(l_forward, r_forward))
        ORK_THROW(ORK("Forward vector calculation was not stable"));
...
{
    stream << ORK("(") << vec.x << ORK(", ") << vec.y << ORK(", ") << vec.z
           << ORK(")");
    return stream;
}
