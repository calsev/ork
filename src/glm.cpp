...
glm::dvec3 rotate(const glm::dvec3&vec, const dunit3&normal, const dunit3&new_normal) {
	...
	if(GLM::antiparallel(normal, new_normal)) {//Special case, needed to prevent undefined cross product
		...
		THROW_MSG(TXT("Only handling orthogonal vectors for now"));
	}
	...
	if(!equal(right, -left))THROW_MSG(TXT("Orthogonal vector calculation was not stable"));
	...
	if(!equal(retval, oriented))THROW_MSG(TXT("Rotation and orientation were not equivalent"));
...
glm::dvec3 proj_on_plane(const glm::dvec3&vec, const GLM::dunit3&normal) {
	...
	if(!equal(l_forward, r_forward))THROW_MSG(TXT("Forward vector calculation was not stable"));
