...
#    define WORK(X) \
        ORK_WIDEN(X) // Wide text, so that searches can identify unhandled unicode
#    define BORK(X) \
        X // Byte text, so that searches can identify unhandled unicode
...
#    ifndef ORK_WARNING
#        if ORK_MSC
#            define ORK_WARNING(MSG) \
                message(__FILE__ \
                        "(" ORK_STR(__LINE__) ") : Warning: " ORK_STR(MSG))
#        elif ORK_GCC
...
#    if ORK_UNICODE
#        define ORK_STR_2_BYTE(ORK_STRING) \
            ork::g_string_converter().wide2byte(ORK_STRING)
#        define ORK_STR_2_WIDE(ORK_STRING) ORK_STRING
#        define ORK_BYTE_2_STR(ORK_STRING) \
            ork::g_string_converter().byte2wide(ORK_STRING)
#        define ORK_WIDE_2_STR(ORK_STRING) ORK_STRING
#    else
#        define ORK_STR_2_BYTE(ORK_STRING) ORK_STRING
#        define ORK_STR_2_WIDE(ORK_STRING) \
            ork::g_string_converter().byte2wide(ORK_STRING)
#        define ORK_BYTE_2_STR(ORK_STRING) ORK_STRING
#        define ORK_WIDE_2_STR(ORK_STRING) \
            ork::g_string_converter().wide2byte(ORK_STRING)
#    endif
