...
#define ORK_THROW(MSG) ORK_STMT(\
	const ork::string backtrace{ORK_BYTE_2_STR(ork::generate_backtrace().str())};\
	ORK_LOG(ork::severity_level::fatal) << MSG << std::endl << std::endl << backtrace << std::endl;\
	...
	throw_msg << std::endl << std::endl << backtrace << std::endl;\
