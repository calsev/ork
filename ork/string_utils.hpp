...
template<typename T>
ORK_INLINE T from_string(const bstring& str);
template<typename T>
ORK_INLINE T from_string(const wstring& str);
...
#define ORK_STRING_CONVERSION(TYPE) \
    ...
    template<> \
    ORK_ORK_EXT(TYPE) \
    from_string<TYPE>(const bstring& str); \
    template<> \
    ORK_ORK_EXT(TYPE) \
    from_string<TYPE>(const wstring& str);
...
ORK_ORK_EXT(bstring)
encode(const unsigned char* str, const size_t size, const encoding enc);
