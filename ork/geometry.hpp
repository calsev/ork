...
ORK_INLINE double mm2inch(double mm)
{
    return mm / 25.4;
}
ORK_INLINE double inch2mm(double inch)
{
    return inch * 25.4;
}
...
bool angles_advance_clockwise(double start_angle, double mid_angle, double end_angle)
{
    ...
    while(mid_angle > start_angle + circle<A>::full)
        mid_angle -= circle<A>::full;

    while(end_angle < start_angle)
        end_angle += circle<A>::full;
    while(end_angle > start_angle + circle<A>::full)
        end_angle -= circle<A>::full;
    ...
    */
    ORK_LOG(severity_level::trace)
        << ORK("start_angle=") << start_angle << ORK(", mid_angle=")
        << mid_angle << ORK(", end_angle=") << end_angle;
...
ORK_ORK_EXT(rotated_view)
rotated(const view& v, const rotation_direction dir, const int rotations);
...
point_on_plane(const glm::dvec3& vec, const orientation axis);
ORK_ORK_EXT(glm::dvec3)
point_on_plane(const glm::dvec3& vec, const GLM::dunit3& normal);


ORK_ORK_EXT(bool)
points_advance_clockwise(const glm::dvec2& begin, const glm::dvec2& end, const glm::dvec2& center);
ORK_ORK_EXT(bool)
points_advance_clockwise(
...
ORK_ORK_EXT(double)
point_on_axis(const glm::dvec3& pos, const GLM::dunit3& axis);
ORK_ORK_EXT(double)
point_on_axis(const glm::dvec3& pos, const glm::dvec3& axis);
...
public:
    ORK_INLINE bounding_box()
        : _min_pt(0., 0., 0.)
        , _max_pt(0., 0., 0.)
    {}
    ORK_ORK_API bounding_box(const glm::dvec3& v1, const glm::dvec3& v2);
    ORK_INLINE bounding_box(const bounding_box& rhs)
        : _min_pt(rhs._min_pt)
        , _max_pt(rhs._max_pt){};
...
ORK_ORK_EXT(bounding_box)
operator|(const bounding_box& lhs, const bounding_box& rhs);
...
public:
    ORK_INLINE interval(const double p1, const double p2)
        : _min(p1 < p2 ? p1 : p2)
        , _max(p1 < p2 ? p2 : p1)
    {}
...
ORK_ORK_EXT(interval)
project_box_on_axis(const glm::dvec3& p1, const glm::dvec3& p2, const GLM::dunit3& axis);
...
public:
    ORK_INLINE const std::vector<segment>& segments() const
    {
        return _segments;
    }
    ORK_INLINE std::vector<segment>& segments()
    {
        return _segments;
    }
    ORK_INLINE void push_back(const segment& s)
    {
