...
enum class border { none, inside, outside, both };
...
public:
    virtual ~exportable() {}
    exportable() {}
    explicit exportable(const bstring& style_)
        : style(style_)
    {}
    exportable(const bstring& style_, const bool span)
        : style(style_)
        , col_span(span)
    {}
...
public:
    pair(const bstring& key_, const bstring& val)
        : key(key_)
        , value(val)
    {}
...
public:
    string() {}
    string(const bstring& style, const bstring& text_)
        : exportable(style)
        , text(text_)
    {}
...
public:
    heading(const bstring& style, const bstring& text, const unsigned level_)
        : string(style, text)
        , level(level_)
    {}
...
public:
    padding() {}
    padding(const int t, const int r, const int b, const int l)
        : top(t)
        , right(r)
        , bottom(b)
        , left(l)
...
public:
    image() {}
    image(const bstring& src, const bstring& alt, const bstring& image_style, const unsigned w, const unsigned h)
        : exportable(image_style)
        , source(src)
        , alt_text(alt)
        , width(w)
        , height(h)
...
public:
    image_style(const bstring& name_, const align a, const padding& pad_)
        : name(name_)
        , alignment(a)
        , pad(pad_)
    {}
...
public:
    line(const bstring& line_style_ /*, const unsigned width, const bool left*/)
        : exportable(line_style_) /*, width_px(width), align_left(left)*/
    {}
...
public:
    label(const bstring& style, const bstring& text_)
        : exportable(style, true)
        , text(text_)
    {}
...
struct ORK_ORK_API table_element {
    static void
    export_html(const exportable* element, const size_t remain_col, std::ostream& out);
};
...
public:
    table(const bstring& style, const unsigned num_column)
        : _table_style(style)
        , _num_column(num_column)
    {}
    table(const bstring& style, const unsigned num_column, const unsigned pad, const unsigned space)
        : padding(pad)
        , spacing(space)
        , _table_style(style)
        , _num_column(num_column)
...
public:
    div(const bstring& div_style)
        : exportable(div_style)
    {}
    ORK_MOVE_ONLY(div);
...
public:
    document() {}
    document(const header& head_, html::body&& body_)
        : head(head_)
        , body(std::move(body_))
    {}
